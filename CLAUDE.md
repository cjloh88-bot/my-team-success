...
A shared internal team operations hub where members log weekly task updates, track work records and statuses, and replace spreadsheet/chat workflows with a live dashboard.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "268130788+cjloh88-bot@users.noreply.github.com"
  git config user.name "cjloh88-bot"
  ```
